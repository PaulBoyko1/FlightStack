...
@pytest.mark.parametrize(("initial_deg", "target_deg"), [([0, 0, 0], [45, 45, 45]), ([60, 20, 40], [-30, 45, 70]), ([-45, 30, -120], [20, -25, 90])])
def test_multiaxis_targets_converge(initial_deg: list[float], target_deg: list[float]) -> None:
    plant = RigidBody(np.diag([0.018, 0.020, 0.035]), attitude=from_euler(*np.deg2rad(initial_deg)))
    target = from_euler(*np.deg2rad(target_deg))
    telemetry = simulate_attitude_step(plant, reference_controller(), target, duration_s=5.0, dt=0.002)
